...
typedef enum MAV_CMD
{
   ...
   MAV_CMD_SET_CAMERA_MODE=530, /* WIP: Set camera running mode. Use NAN for reserved values and values you don't want to change. |Camera mode (CAM_MODE 0: photo/image mode, 1: video mode)| Reserved (all remaining params)|  */
   MAV_CMD_IMAGE_START_CAPTURE=2000, /* WIP: Start image capture sequence. Sends CAMERA_IMAGE_CAPTURED after each capture. Use NAN for reserved values. |Duration between two consecutive pictures (in seconds)| Number of images to capture total - 0 for unlimited capture| Reserved|  */
   MAV_CMD_IMAGE_STOP_CAPTURE=2001, /* WIP: Stop image capture sequence |Reserved|  */
   ...
   MAV_CMD_VIDEO_START_CAPTURE=2500, /* WIP: Starts video capture (recording). Use NAN for reserved values. |Frequency CAMERA_CAPTURE_STATUS messages should be sent while recording (0 for no messages, otherwise frequency in Hz)| Reserved|  */
   MAV_CMD_VIDEO_STOP_CAPTURE=2501, /* WIP: Stop the current video capture (recording). Use NAN for reserved values. |Reserved|  */
