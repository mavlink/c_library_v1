...
typedef enum MAV_CMD
{
   ...
   MAV_CMD_NAV_VTOL_TAKEOFF=84, /* Takeoff from ground using VTOL mode, and transition to forward flight with specified heading. |1: Empty| 2: Front transition heading.| 3: Empty| 4: Yaw angle. NaN for unchanged.| 5: Latitude| 6: Longitude| 7: Altitude|  */
   ...
   MAV_CMD_DO_LAST=240, /* NOP - This command is only used to mark the upper limit of the DO commands in the enumeration |1: Empty| 2: Empty| 3: Empty| 4: Empty| 5: Empty| 6: Empty| 7: mpty|  */
   ...
   MAV_CMD_PREFLIGHT_SET_SENSOR_OFFSETS=242, /* Set sensor offsets. This command will be only accepted if in pre-flight mode. |1: Sensor to adjust the offsets for: 0: gyros, 1: accelerometer, 2: magnetometer, 3: barometer, 4: optical flow, 5: second magnetometer, 6: third magnetometer| 2: (or generic dimension 1), in the sensor's raw units| 3: Y axis offset (or generic dimension 2), in the sensor's raw units| 4: Z axis offset (or generic dimension 3), in the sensor's raw units| 5: Generic dimension 4, in the sensor's raw units| 6: Generic dimension 5, in the sensor's raw units| 7: Generic dimension 6, in the sensor's raw units|  */
   ...
   MAV_CMD_STORAGE_FORMAT=526, /* Format a storage medium. Once format is complete, a STORAGE_INFORMATION message is sent. Use the command's target_component to target a specific component's storage. |1: for first, 2 for second, etc.)| 2: 0: No action 1: Format storage| 3: Reserved (all remaining params)| 4: Reserved (default:0)| 5: Reserved (default:0)| 6: Reserved (default:0)| 7: Reserved (default:0)|  */
   ...
   MAV_CMD_REQUEST_VIDEO_STREAM_STATUS=2505, /* Request video stream status (VIDEO_STREAM_STATUS) |1: Video Stream ID (0 for all streams, 1 for first, 2 for second, etc.)| 2: params)| 3: Reserved (default:0)| 4: Reserved (default:0)| 5: Reserved (default:0)| 6: Reserved (default:0)| 7: Reserved (default:0)|  */
   ...
   MAV_CMD_PAYLOAD_PREPARE_DEPLOY=30001, /* Deploy payload on a Lat / Lon / Alt position. This includes the navigation to reach the required release position and velocity. |1: Operation mode. 0: prepare single payload deploy (overwriting previous requests), but do not execute it. 1: execute payload deploy immediately (rejecting further deploy commands during execution, but allowing abort). 2: add payload deploy to existing deployment list.| 2: Desired approach vector in compass heading. A negative value indicates the system can define the approach vector at will.| 3: Desired ground speed at release time. This can be overridden by the airframe in case it needs to meet minimum airspeed. A negative value indicates the system can define the ground speed at will.| 4: position. A negative value indicates the system can define the clearance at will.| 5: Latitude unscaled for MISSION_ITEM or in 1e7 degrees for MISSION_ITEM_INT| 6: Longitude unscaled for MISSION_ITEM or in 1e7 degrees for MISSION_ITEM_INT| 7: Altitude (MSL), in meters|  */
